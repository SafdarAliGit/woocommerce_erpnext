...
def batch_sync_items():
    # sync erpnext items to WooCommerce product in batches of 25
    # run from the terminal to see messages
    ...
    def log(res):
        if not res.get("error"):
            ...
            print(res)
            # collect all errors and email to the user for failed imports.
...
def sync_product_categories(item_group=None):
    ...
    # Does not handle nested item groups
    parameter = {}
...
def on_delete_item(doc, method=None):
    if doc.woocommerce_id:
        r = get_connection().delete("products/" + str(doc.woocommerce_id))
        print(r)
